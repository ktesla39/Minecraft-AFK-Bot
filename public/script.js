...
function renderLine(kind, entry) {
  ...
  if (kind === 'console') {
    line.classList.add(`log-line--${entry.level === 'error' ? 'error' : entry.level === 'warn' ? 'warn' : 'info'}`);
    ...
    if (entry.username) {
      ...
      if (entry.username === currentBotUsername) line.classList.add('log-line--chat-self');
    }
    body.textContent = entry.message;
  } else if (kind === 'errors') {
    line.classList.add('log-line--error');
    body.textContent = entry.detail ? `${entry.message}: ${entry.detail}` : entry.message;
...
function matchesFilter(kind, entry, query) {
  ...
  const haystack = [
    entry.message, entry.detail, entry.username, entry.event,
...
function renderView(kind) {
  ...
  if (items.length === 0) {
    ...
    empty.textContent = query ? 'No entries match your filter.' : 'Nothing here yet.';
...
function applyState(state) {
  ...
  statPing.textContent = state.status === 'online' && state.ping != null ? `${state.ping} ms` : '—';
  statUptime.textContent = state.status === 'online' ? formatUptime(state.uptimeMs) : '—';
...
document.querySelector('.tab[data-tab="errors"]').addEventListener('click', () => {
  errorCount = 0;
